...
# Load data
data = np.genfromtxt('multi_1.csv', skip_header=1, delimiter=',', dtype=np.float64)
...
for i in range(3):
    ...
    with open(f"error_multi_1_{i+1}.txt", "w") as f:
...
fig.suptitle("Tripple Aqua Localization Sample 1", fontsize=14)
# Adjust layout
plt.tight_layout()
plt.savefig("./plots/mutli_1.png", dpi=300, bbox_inches="tight")  # Saves as PNG with high resolution
"""